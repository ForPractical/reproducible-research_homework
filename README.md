...
(a) I observed two random paths of the Brownian motion generated by the code. As I run the code repeatedly, different walking paths from the previous ones are observed. Thus, randomness is involved in this model so that we can get different results every time we run the code. The random movements can occur in all directions in the two-dimensional plane based on the angle randomly selected at every step. The time variable reprensented by a gradient of colors indicates the directions and orders of the paths. The movements can begin and end at any random points at the 2D plane, and the paths form a continous line connecting the random points generated at every step.
